...
public:
    ...
    sensor_msgs::Imu imuConverter(const sensor_msgs::Imu& imu_in)
    {
        ...
        // Step 1: 把IMU的加速度、角速度转到LiDAR坐标系下，即acc_lidar = R_lidar_imu * acc_imu
        // 这里把imu的数据旋转到前左上坐标系下，可以参考https://github.com/TixiaoShan/LIO-SAM/issues/6
        //; 注意：上面的注释并不准确，这里不是把IMU数据转到前左上坐标系下，而是要转到LiDAR坐标系下，
        //;      只不过对于作者的安装设备来说，LiDAR和IMU的坐标轴是平行的，所以外参中就是单位阵
        // rotate acceleration
        ...
        // 这是一个九轴imu，因此还会有姿态信息
        //; 1.关于什么是六轴、九轴IMU：https://zhuanlan.zhihu.com/p/344884686
        //;   其实普通的六轴IMU，通过内部的运算单元或者外部自己使用算法处理，就可以得到姿态角，只不过yaw角度
        //;   不是绝对角度，而是相对角度。而且由于yaw角度只能用陀螺仪积分得到，所以会有很大的漂移。而九轴IMU
        //;   由于加入了磁力计，yaw角度没有漂移，而且可以得到绝对的yaw角。
        //; 2.关于为什么叫做三轴磁力计：https://m.elecfans.com/article/1902057.html
        //;   其实本质上磁力计只能得到yaw角度，但是还叫它三轴磁力计是因为传感器内部的布置是有三个轴的，但是
        //;   实际上只能得到yaw角度，pitch/roll还是靠六轴IMU的加速度和角速度融合得到的
        Eigen::Quaterniond q_from(imu_in.orientation.w, imu_in.orientation.x, imu_in.orientation.y, imu_in.orientation.z);
        
        // Step 2: 把IMU的姿态转成LiDAR的姿态，即R_w_lidar = R_w_imuquat * R_imuquat_lidar
        //  但是由于本文作者使用的IMU，其加速度、角速度坐标系和姿态坐标系不同，所以有两个旋转外参
